<script lang="ts">
  ...
  import { setEditing, updateEditables, type EditController } from "./edit-controller";
  import { type RenderArgs, type RenderSpec } from "./template";
  import {
    createMessage,
    handleMessage,
    type AppMessage,
  ...
  const width = $derived(spec.size?.[0] || innerWidth);
  const height = $derived(spec.size?.[1] || innerHeight);
...
<svelte:window onmessage={onMessage} bind:innerHeight bind:innerWidth />

{#if width && height}
  <div id="ad" style:height="{height}px" style:width="{width}px">
    <Template {args} />
  </div>
{/if}
